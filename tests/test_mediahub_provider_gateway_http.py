...
def test_provider_order_is_deterministic():
    assert [p.name for p in module.PROVIDERS] == ["openrouter", "opper", "continuum"]


def test_capability_matrix_is_explicit():
    assert "responses" in module.CAPABILITIES["openrouter"]
...
def test_upstream_paths_and_credentials(tmp_path, monkeypatch):
    for name in ("mediahub-openrouter", "mediahub-opper", "mediahub-continuum"):
        (tmp_path / name).write_text(name + "-secret", encoding="utf-8")
    monkeypatch.setenv("CREDENTIALS_DIRECTORY", str(tmp_path))
    assert module.upstream("openrouter", "/v1/responses")[1] == "/api/v1/responses"
...
def test_policy_blocked_does_not_retry_same_provider():
    ...
    decision = gateway.failover("openrouter", 403, "Access denied by security policy")
