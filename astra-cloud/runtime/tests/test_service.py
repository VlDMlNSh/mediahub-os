...
from astra_cloud.runtime.service import AstraCloudService, TaskState
...
BASE = {
    ...
    "request_id": "req-1",
    "session_id": "sess-1",
    "user_command": "inspect repository",
    "approval_state": "not_required",
}


class AstraCloudServiceTests(unittest.TestCase):
    def test_unconfigured_provider_blocks_without_secret_leak(self):
    ...
    def test_rejected_task_is_not_executed(self):
        task = dict(BASE, approval_state="rejected")
        with self.assertRaisesRegex(ValueError, "task_rejected"):
            AstraCloudService(lambda _: "must-not-run").submit(task)

    def test_invalid_schema_is_rejected(self):
        task = dict(BASE, schema_id="other")
