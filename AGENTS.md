# MediaHub governance override

These project constraints are mandatory and take precedence over generic Godmode shipping/automation guidance:

- State Authority is never mutated by Godmode agents.
- Production, release authorization, secrets/credentials, VPN activation, and merge are outside agent authority.
- All changes require MediaHub verification and remain reviewable; human authorization is required for merge/release/production.
- Cloud/provider unavailability is fail-closed; never bypass authentication or fabricate provider availability.
- Godmode agents are advisory/subordinate engineering workers and must respect task leases and isolated worktrees.
