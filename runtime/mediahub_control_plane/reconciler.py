...
class ControlPlaneReconciler:
    ...
    def reconcile_once(self, now: float | None = None) -> tuple[str, ...]:
        ...
        for task in tasks:
            if task.status in {TaskStatus.PENDING,TaskStatus.BLOCKED,TaskStatus.RETRY_WAIT} and dependencies_satisfied(task,completed):
                self.repository.update_task(replace(task,status=TaskStatus.READY)); changed.append(task.task_id)
