...
AGENTS: Mapping[str, NativeAgentSpec] = {
    "codex": NativeAgentSpec("codex", "openai", Path("/usr/local/bin/codex"), "OPENAI_API_KEY", "OPENAI_BASE_URL"),
    "claude": NativeAgentSpec("claude", "anthropic", Path("/usr/local/bin/claude"), "ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL"),
}
...
def build_command(agent: str, model: str, prompt: str) -> tuple[str, ...]:
    ...
    if agent == "codex":
        return (str(spec.executable), "exec", "--json", "--ephemeral", "--skip-git-repo-check",
                "--sandbox", "workspace-write", "--model", model, prompt)
    return (str(spec.executable), "-p", "--output-format", "json", "--model", model, prompt)
