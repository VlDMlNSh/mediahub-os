...
def qualify_ollama(model: str, *, url: str = "http://127.0.0.1:11434/api/chat") -> dict[str, Any]:
    ...
    if coding_ok:
        with tempfile.TemporaryDirectory(prefix="mediahub-model-qualification-") as tmp:
            ...
            applied = subprocess.run(["patch", "-p1", "--forward", str(patch)], cwd=root, text=True, capture_output=True, check=False)
            patch_ok = applied.returncode == 0 and target.read_text(encoding="utf-8") == "VALUE = 2\n"
            if patch_ok:
                test = subprocess.run(["python3", "-m", "py_compile", str(target)], cwd=root, text=True, capture_output=True, check=False)
                test_ok = test.returncode == 0
    qualified = coding_ok and patch_ok and test_ok
    record = ModelRecord(provider="ollama", model=model, version=models[model].get("modified_at", ""), digest=digest,
    ...
    return {"status": record.qualification_status, "marker": marker_text, "marker_ms": marker_ms,
            "coding_ms": coding_ms, "digest": digest, "digest_sha256": hashlib.sha256(digest.encode()).hexdigest(),
            "coding_ok": coding_ok, "patch_ok": patch_ok, "test_ok": test_ok, "record": record}
