...
PYTHONPATH=runtime:. "$ROOT/.autonomous/venv/bin/python" -m pytest -q >"$OUT/pytest.log" 2>&1
PYTHONPATH=runtime:. "$ROOT/.autonomous/venv/bin/python" -m pytest -q tests/security >"$OUT/security-pytest.log" 2>&1
...
"$ROOT/.autonomous/venv/bin/pip-audit" --local >"$OUT/pip-audit.log" 2>&1
